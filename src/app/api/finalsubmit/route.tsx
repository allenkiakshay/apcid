...
export async function GET(req: Request) {
  try {
    ...
    if (excelfile && fs.existsSync(excelfile.oexcelurl) && fs.existsSync(excelfile.pexcelurl)) {
    ...
    if (pptfile && fs.existsSync(pptfile.oppturl) && fs.existsSync(pptfile.pppturl)) {
    ...
    if (wordfile && fs.existsSync(wordfile.owordurl) && fs.existsSync(wordfile.pwordurl)) {
    ...
    if (textfile && fs.existsSync(textfile.otexturl) && fs.existsSync(textfile.ptexturl)) {
    ...
    if (availableFiles.length === 0) {
      return NextResponse.json(
        { error: "No valid files found for submission. Please upload at least one file." },
        { status: 400 }
      );
    }

    console.log(`Submitting ${availableFiles.length} files for user ${fetched_user.hallticket}:`);
    ...
    return NextResponse.json(
      { error: "Failed to process final submission", details: error.message },
      { status: 500 }
    );
  } finally {
